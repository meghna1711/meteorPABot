...
Meteor.startup(function(){
    ...
    Session.setDefault("roomname", "admin");
});
...
Template.dashboard.events({
   'submit form'  : function(e){
       ...
       var el = document.getElementById("msg"),
           user = Profile.find({userId : Meteor.userId()}).fetch();
       Messages.insert({user: user[0].given_name + user[0].family_name , msg: el.value, ts: new Date(), picture:user[0].picture ,room: Session.get("roomname")});
       el.value = "";
       el.focus();
   }
});
